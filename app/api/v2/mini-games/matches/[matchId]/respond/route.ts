...
export async function POST(request: Request, context: RouteParams<{ matchId: string }>) {
  return withActor(request, async (actor) => {
    ...
    return runMutationCommand(actor, 'respondToMiniGameChallenge', { ...body, matchId }, {
      crewId: body.crewId,
      eventType: body.accepted === false ? 'mini_game.challenge_declined' : 'mini_game.challenge_accepted',
